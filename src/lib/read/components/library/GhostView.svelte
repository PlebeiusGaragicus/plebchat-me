<script lang="ts">
	...
	import { compareCfi, HIGHLIGHT_COLORS } from '$lib/read/epub/service.js';
...
<div class="flex h-full flex-col" data-testid="ghost-view">
	<div class="flex items-center gap-2 border-b border-border px-3 py-1.5">
		<button class="rounded p-1.5 hover:bg-accent" title="Back to library" onclick={back}>
		...
		<div class="mx-auto max-w-2xl p-6">
			<div class="mb-6 flex gap-4">
				...
				<div class="min-w-0">
					...
					<p class="text-sm text-muted-foreground">{book?.creator}</p>
					...
					{#if missing}
						<button
							class="mt-3 flex items-center gap-1.5 rounded-lg bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
							disabled={busy}
			...
			{#if sorted.length === 0}
				<p class="py-8 text-center text-sm text-muted-foreground">
					No annotations for this book yet.
				</p>
			{:else}
						...
						<div class="rounded-lg border border-border p-2.5">
							<div class="flex items-start gap-2">
								...
								<div class="min-w-0 flex-1">
									...
									{#if anno.note}
										<p class="mt-1 text-xs text-muted-foreground">{anno.note}</p>
									{/if}
								</div>
								{#if anno.shared}
